...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/pushkarsingh/Documents/01 University/03 Self Prioritisation Research/experiment/Psychopy UGRP Experiment_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    while continueRoutine:
        ...
        if triangle_path.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'triangle_path.started')
            # update status
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 3.5:
            ...
            if label.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > label.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if shape_1.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'shape_1.started')
                # update status
            ...
            if shape_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > shape_1.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
                    shape_1.tStop = t  # not accounting for scr refresh
                    shape_1.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'shape_1.stopped')
                    # update status
    ...
    for thisTrial_3 in trials_3:
        ...
        while continueRoutine and routineTimer.getTime() < 3.5:
            ...
            if distractor_block1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > distractor_block1.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if blockone_label.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blockone_label.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if block1_shape.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'block1_shape.started')
                # update status
            ...
            if block1_shape.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > block1_shape.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
                    block1_shape.tStop = t  # not accounting for scr refresh
                    block1_shape.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'block1_shape.stopped')
                    # update status
    ...
    _key_resp_6_allKeys = []
    # keep track of which components have finished
    break_2Components = [text_5, key_resp_6]
    ...
    for thisTrial_4 in trials_4:
        ...
        while continueRoutine and routineTimer.getTime() < 3.5:
            ...
            if distractor_block1_3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > distractor_block1_3.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if blockone_label_3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blockone_label_3.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp_8.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp_8.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if block1_shape_3.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'block1_shape_3.started')
                # update status
            ...
            if block1_shape_3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > block1_shape_3.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
                    block1_shape_3.tStop = t  # not accounting for scr refresh
                    block1_shape_3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'block1_shape_3.stopped')
                    # update status
        ...
        if key_resp_8.corr == 1:
            ...
            feedback_blockOne = "incorrect"
        text_6.setText(feedback_blockOne)
        # keep track of which components have finished
