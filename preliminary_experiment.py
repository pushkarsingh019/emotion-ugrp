...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/pushkarsingh/Documents/01 University/03 Self Prioritisation Research/experiment/preliminary_experiment.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    fixation = visual.ShapeStim(
        ...
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=0.0, interpolate=True)
    # Run 'Begin Experiment' code from show_elements
    ...
    label_triangle = visual.TextStim(win=win, name='label_triangle',
            text='You',
            font='Open Sans',
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('self_other_starting.started', globalClock.getTime())
    key_instruct_3.keys = []
    key_instruct_3.rt = []
    ...
    for thisComponent in self_other_startingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('self_other_starting.stopped', globalClock.getTime())
    # check responses
    ...
    # keep track of which components have finished
    trialComponents = [fixation, triangle, label_triangle, circle]
    ...
    while continueRoutine:
        ...
        if triangle.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *label_triangle* updates
        
        # if label_triangle is starting this frame...
        if label_triangle.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
            label_triangle.frameNStart = frameN  # exact frame index
            label_triangle.tStart = t  # local t and not account for scr refresh
            label_triangle.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(label_triangle, 'tStartRefresh')  # time at next scr refresh
            # update status
            label_triangle.status = STARTED
            label_triangle.setAutoDraw(True)
        
        # if label_triangle is active this frame...
        if label_triangle.status == STARTED:
            # update params
            pass
        ...
        if circle.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
